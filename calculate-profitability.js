...
/* =========================
   CALCULATOR
========================= */
function calculateProfitability(tradeAmountUSDC, spreadPercent, maxSlippagePercent = 0.5, priorityFeeMicroLamports = 50000) {
    console.log("\n" + "=".repeat(80));
    console.log("💰 ARBITRAGE PROFITABILITY CALCULATOR");
    console.log("=".repeat(80));
    // Input
    const tradeAmount = new decimal_js_1.default(tradeAmountUSDC);
    const spread = new decimal_js_1.default(spreadPercent).div(100);
    console.log("\n📥 INPUT:");
    ...
    const totalGasSOL = new decimal_js_1.default(totalGasLamports).div(1e9);
    ...
    // Jito tip
    let jitoTipUSD = new decimal_js_1.default(0);
    if (USE_JITO) {
        const jitoTipSOL = new decimal_js_1.default(JITO_TIP_LAMPORTS).div(1e9);
    ...
    if (isProfitable) {
        ...
        if (roi.gte(0.1)) {
            const tradesFor1Dollar = new decimal_js_1.default(1).div(netProfit);
            console.log(`   Trades for $1 profit: ${tradesFor1Dollar.toFixed(1)} trades`);
        }
        if (roi.gte(0.01)) {
            const tradesFor10Dollars = new decimal_js_1.default(10).div(netProfit);

...
class CsvLogger {
    constructor(logDir = './logs') {
        ...
        this.logFilePath = path_1.default.join(logDir, `trades_${date}.csv`);
        this.summaryFilePath = path_1.default.join(logDir, `summary_${date}.csv`);
    ...
    writeSummary(stats) {
        ...
        fs_1.default.writeFileSync(this.summaryFilePath, summary);
     ...
     * Read recent trades
     */
    readRecentTrades(count = 10) {
        if (!fs_1.default.existsSync(this.logFilePath)) {
            return [];
        }
        const lines = fs_1.default.readFileSync(this.logFilePath, 'utf8').split('\n');
